...
app.get('/invoices/download/:saleId', isAuthenticated, async (req, res) => {
  try {
    ...
    // Page dimensions for better page break handling
    const PAGE_HEIGHT = 842; // A4 height in points
    const BOTTOM_LIMIT = PAGE_HEIGHT - 50; // Bottom margin 50
    ...
    // Helper function to check if we need a new page (now using currentY)
    ...
    if (admin?.logo && fs.existsSync(path.join(__dirname, 'public', admin.logo))) {
      try {
        doc.image(path.join(__dirname, 'public', admin.logo), 50, 50, { width: 80, height: 80, fit: [80, 80] });
        logoHeight = 80;
      } catch (error) {
        console.error('Error loading logo:', error);
      }
    ...
    // Admin login text below logo (fixed position to avoid overlap)
    const generatedByY = 50 + logoHeight + 5;
    doc.fillColor('#666666')
       .fontSize(8)
       .font('Helvetica')
       .text(`Generated by: ${req.session.admin}`, 50, generatedByY, { align: 'left' });

    // Set currentY after generated by text
    currentY = generatedByY + 20;

    // Invoice Title - moved to right side
    doc.fillColor(primaryColor)
       .fontSize(24)
       .font('Helvetica-Bold')
       .text('INVOICE', 400, 50, { align: 'right' });
    ...
    // Header separator line
    doc.moveTo(50, currentY).lineTo(550, currentY).strokeColor(primaryColor).lineWidth(2).stroke();

    currentY += 20;

    // Business Information Box
    drawColoredBox(50, currentY, 230, 80, accentColor);
    doc.fillColor(textColor)
       .fontSize(10)
       .font('Helvetica-Bold')
       .text('FROM:', 60, currentY + 10);
    ...
    doc.font('Helvetica')
       ...
       .text(admin?.email || 'N/A', 60, currentY + 38)
       .text(admin?.phone || 'N/A', 60, currentY + 51)
       .text(`Currency: ${admin?.currency || 'USD'}`, 60, currentY + 64);

    // Invoice Details Box
    drawColoredBox(300, currentY, 250, 80, accentColor);
    doc.fillColor(textColor)
       .fontSize(10)
       .font('Helvetica-Bold')
       .text('INVOICE DETAILS:', 310, currentY + 10);
    ...
    doc.font('Helvetica')
       ...
       .text(`Date: ${new Date(sale.date).toLocaleDateString()}`, 310, currentY + 38)
       .text(`Due: ${new Date(sale.date).toLocaleDateString()}`, 310, currentY + 51)
       .text(`Status: ${sale.paymentStatus || 'Pending'}`, 310, currentY + 64);
    ...
    currentY = checkPageBreak(250);

    // Items Table Header
    drawColoredBox(50, currentY, 500, 25, primaryColor, 'ITEMS DETAILS');
    currentY += 30;

    // Table Headers - UPDATED: Increased Amount column width
    const colWidths = [200, 70, 80, 70, 80]; // Increased last column from 60 to 80
    const headerY = currentY;
    ...
    doc.text('Description', 50, headerY, { width: colWidths[0] });
    doc.text('Qty', 250, headerY, { width: colWidths[1], align: 'center' });
    doc.text('Unit Price', 320, headerY, { width: colWidths[2], align: 'right' });
    doc.text('VAT', 400, headerY, { width: colWidths[3], align: 'right' });
    doc.text('Amount', 470, headerY, { width: colWidths[4], align: 'right' }); // Updated position
    ...
    if (sale.items && Array.isArray(sale.items)) {
      sale.items.forEach((item, index) => {
        // Check for page break before each row (updated to use BOTTOM_LIMIT)
        if (itemsY + 20 > BOTTOM_LIMIT) {
          doc.addPage();
          itemsY = 50;
          ...
          doc.text('Description', 50, itemsY, { width: colWidths[0] });
          doc.text('Qty', 250, itemsY, { width: colWidths[1], align: 'center' });
          doc.text('Unit Price', 320, itemsY, { width: colWidths[2], align: 'right' });
          doc.text('VAT', 400, itemsY, { width: colWidths[3], align: 'right' });
          doc.text('Amount', 470, itemsY, { width: colWidths[4], align: 'right' });
          
          doc.moveTo(50, itemsY + 12).lineTo(550, itemsY + 12).strokeColor(borderColor).lineWidth(1).stroke();
          itemsY += 20;
        }
        ...
        doc.text(String(quantity), 250, itemsY, { width: colWidths[1], align: 'center' });
        doc.text(`${admin?.currency || '$'} ${formatCurrency(unitCost)}`, 320, itemsY, { width: colWidths[2], align: 'right' });
        
        // VAT column
        if (isVatable && vatAmount > 0) {
          doc.fillColor(secondaryColor)
             .text(`${vatRate}%`, 400, itemsY, { width: colWidths[3], align: 'right' });
        } else {
          doc.fillColor('#666666')
             .text('N/A', 400, itemsY, { width: colWidths[3], align: 'right' });
        }
        
        doc.fillColor(textColor)
           .text(`${admin?.currency || '$'} ${formatCurrency(totalCost)}`, 470, itemsY, { width: colWidths[4], align: 'right' });
    ...
    currentY = itemsY + 30; // Increased spacing after table
    ...
    currentY += 50; // SIGNIFICANTLY increased spacing after totals (from 40 to 50)

    // Check page break before payment information
    currentY = checkPageBreak(120);

    // Payment Information
    drawColoredBox(50, currentY, 500, 80, highlightColor);
    doc.fillColor(textColor)
       .fontSize(10)
       .font('Helvetica-Bold')
       .text('PAYMENT INFORMATION:', 60, currentY + 10);
    ...
    doc.font('Helvetica')
       ...
       .text(`Status: ${sale.paymentStatus || 'Pending'}`, 60, currentY + 38)
       .text(`Date: ${new Date(sale.date).toLocaleDateString()}`, 60, currentY + 51)
       .text(`Invoice ID: ${saleId.slice(-8)}`, 60, currentY + 64);
    ...
    currentY += 100; // Fixed typo: increased spacing after payment information (from 100 to 120)

    // Check page break before additional comments
    if (sale.additionalComments && sale.additionalComments.trim()) {
      // Calculate how much space we need for the comments (removed min limit to allow full height)
      const commentHeight = doc.heightOfString(sale.additionalComments, { 
        width: 490, 
        align: 'left',
        lineGap: 2
      }) + 20;

      // Check if we need a new page for comments (using full height)
      currentY = checkPageBreak(commentHeight + 50); // +50 for header and spacing
      
      drawColoredBox(50, currentY, 500, 25, secondaryColor, 'ADDITIONAL COMMENTS');
      currentY += 30;

      // Comments box with border
      doc.rect(50, currentY, 500, commentHeight).strokeColor(borderColor).lineWidth(1).stroke();
      
      // Display existing comments
      doc.fillColor(textColor)
         .fontSize(9)
         .font('Helvetica')
         .text(sale.additionalComments, 55, currentY + 10, { 
           width: 490, 
           align: 'left',
           lineGap: 2
         });

      currentY += commentHeight + 30; // Dynamic spacing based on comment height
    }

    // Footer - with proper spacing check
    const footerHeight = 60;
    currentY = checkPageBreak(footerHeight);
    ...
    doc.fillColor('#666666')
       ...
       .text(`Generated on: ${new Date().toLocaleString()}`, 50, footerY + 22, { width: 500, align: 'center' })
       .text('Shed: Inventory, Invoices and More', 50, footerY + 34, { width: 500, align: 'center' });
