...
app.get('/invoices/download/:saleId', isAuthenticated, async (req, res) => {
  try {
    ...
    const notesBackground = '#F9F9F9';
    const balanceBg = '#E8F5E9';
    ...
    if (admin?.logo && fs.existsSync(path.join(__dirname, 'public', admin.logo))) {
      try {
        doc.image(path.join(__dirname, 'public', admin.logo), 50, y, { width: 70 });
      } catch (err) {
        console.error('Error loading logo:', err);
      }
    }
    ...
    doc.fillColor(primaryColor)
      .fontSize(14)
      .font('Helvetica-Bold')
      .text(admin?.businessName || 'Business Name', 50, y + 80);

    doc.fontSize(10).font('Helvetica')
      .text(admin?.address || '', 50, y + 95)
      .text(admin?.email || '', 50, y + 108)
      .text(admin?.phone || '', 50, y + 121);
    ...
    doc.fontSize(10).font('Helvetica').fillColor(primaryColor)
      .text(`Invoice #: ${saleId.slice(-8)}`, 350, 85, { align: 'right' })
      .text(`Invoice Date: ${new Date(sale.date).toLocaleDateString()}`, 350, 100, { align: 'right' })
      .text(`Terms: Due on Receipt`, 350, 115, { align: 'right' })
      .text(`Due Date: ${new Date(sale.date).toLocaleDateString()}`, 350, 130, { align: 'right' });

    y += 160;
    doc.moveTo(50, y).lineTo(550, y).strokeColor(borderGray).lineWidth(1).stroke();
    y += 20;

    // === FROM / BILL TO ===
    doc.font('Helvetica-Bold').fillColor(primaryColor).fontSize(10).text('FROM', 50, y);
    doc.font('Helvetica').fillColor(textColor).fontSize(10)
      .text(admin?.businessName || '', 50, y + 12)
      .text(admin?.email || '', 50, y + 24)
      .text(admin?.phone || '', 50, y + 36)
      .text(`Currency: ${admin?.currency || 'USD'}`, 50, y + 48);

    doc.font('Helvetica-Bold').fillColor(primaryColor).fontSize(10).text('BILL TO', 400, y, { align: 'right' });
    doc.font('Helvetica').fillColor(textColor).fontSize(10)
      .text(sale.customerName || '', 400, y + 12, { align: 'right' })
      .text(sale.phoneNumber || '', 400, y + 24, { align: 'right' })
      .text(sale.email || '', 400, y + 36, { align: 'right' });

    y += 80;
    doc.moveTo(50, y).lineTo(550, y).strokeColor(borderGray).stroke();
    y += 15;

    // === Subject ===
    if (sale.subject) {
      doc.font('Helvetica-Bold').fillColor(primaryColor).text('Subject:', 50, y);
      doc.font('Helvetica').fillColor(textColor).text(sale.subject, 110, y);
      y += 20;
    }

    // === Table Header ===
    doc.rect(50, y, 500, 25).fill(greenColor);
    doc.fillColor('#ffffff').fontSize(10).font('Helvetica-Bold');
    doc.text('#', 55, y + 7, { width: 20 });
    doc.text('Description', 80, y + 7, { width: 180 });
    doc.text('Qty', 280, y + 7, { width: 50, align: 'right' });
    doc.text('Rate', 350, y + 7, { width: 80, align: 'right' });
    doc.text('Amount', 450, y + 7, { width: 80, align: 'right' });
    y += 25;

    // === Table Rows ===
    doc.font('Helvetica').fontSize(10).fillColor(textColor);
    const calcHeight = (txt, width) => Math.max(25, doc.heightOfString(txt, { width }) + 8);

    sale.items.forEach((item, i) => {
      const h = calcHeight(item.itemName || '', 180);
      const bg = i % 2 === 0 ? lightGray : '#FFFFFF';
      doc.rect(50, y, 500, h).fill(bg);
      doc.fillColor(textColor);
      doc.text(String(i + 1), 55, y + 7);
      doc.text(item.itemName || '', 80, y + 7, { width: 180 });
      doc.text(String(item.quantity || 0), 280, y + 7, { width: 50, align: 'right' });
      doc.text(`${admin?.currency || '$'} ${formatCurrency(item.unitCost)}`, 350, y + 7, { width: 80, align: 'right' });
      doc.text(`${admin?.currency || '$'} ${formatCurrency(item.totalCost)}`, 450, y + 7, { width: 80, align: 'right' });
      y += h;
    });
    ...
    const colX = { label: 350, value: 450 };
    doc.fontSize(10).fillColor(primaryColor).font('Helvetica');
    doc.text('Sub Total', colX.label, y, { width: 80, align: 'right' });
    doc.text(`${admin?.currency || '$'} ${formatCurrency(sale.subtotalAmount || sale.totalAmount || 0)}`, colX.value, y, { width: 80, align: 'right' });
    y += 15;

    if (sale.totalVatAmount > 0) {
      doc.text(`VAT (${sale.vatRate || admin?.vatRate || 0}%)`, colX.label, y, { width: 80, align: 'right' });
      doc.text(`${admin?.currency || '$'} ${formatCurrency(sale.totalVatAmount)}`, colX.value, y, { width: 80, align: 'right' });
      y += 15;
    }

    doc.font('Helvetica-Bold').fontSize(11).fillColor(primaryColor);
    doc.text('Total', colX.label, y, { width: 80, align: 'right' });
    doc.text(`${admin?.currency || '$'} ${formatCurrency(sale.totalAmount)}`, colX.value, y, { width: 80, align: 'right' });
    y += 40;

    // === Balance Due + Payment Info Row (with divider) ===
    const rowHeight = 70;
    const dividerX = 310;
    doc.rect(50, y, 500, rowHeight).fill(balanceBg);
    const balanceY = y + 12;

    doc.font('Helvetica-Bold').fontSize(13).fillColor(primaryColor)
      .text('Balance Due', 60, balanceY);
    doc.font('Helvetica-Bold').fontSize(13).fillColor(greenColor)
      .text(`${admin?.currency || '$'} ${formatCurrency(sale.totalAmount)}`, 60, balanceY + 18);

    doc.moveTo(dividerX, y + 10)
      .lineTo(dividerX, y + rowHeight - 10)
    ...
    doc.font('Helvetica-Bold').fontSize(11).fillColor(primaryColor)
      .text('PAYMENT INFORMATION', dividerX, balanceY, { width: 240, align: 'right' });
    doc.font('Helvetica').fillColor(textColor).fontSize(10);
    doc.text(bank.bankAccountName || admin?.businessName || '', dividerX, balanceY + 15, { width: 240, align: 'right' });
    doc.text(bank.bankName || admin?.bankName || '', dividerX, balanceY + 27, { width: 240, align: 'right' });
    doc.text(bank.accountNumber || admin?.accountNumber || '', dividerX, balanceY + 39, { width: 240, align: 'right' });

    y += rowHeight + 20;

    // === Notes Section ===
    if (sale.additionalComments && sale.additionalComments.trim()) {
      const commentHeight = doc.heightOfString(sale.additionalComments, { width: 450 }) + 20;
      doc.rect(50, y, 500, commentHeight).fill(notesBackground);
      doc.font('Helvetica-Bold').fillColor(primaryColor).text('Notes:', 60, y + 10);
      doc.font('Helvetica').fillColor(textColor)
        .text(sale.additionalComments, 100, y + 10, { width: 430 });
      y += commentHeight + 10;
    }

    // === Footer with Logo, Text, and Clickable Link ===
    y += 30;
    doc.moveTo(50, y).lineTo(550, y).strokeColor(borderGray).lineWidth(1).stroke();
    y += 15;
    ...
    if (fs.existsSync(logoPath)) {
      try {
        doc.image(logoPath, 50, y, { width: 40, height: logoHeight });
        footerLogoWidth = 45;
      } catch (err) {
    ...
    // Text aligned horizontally with the logo
    const footerTextY = y + 5;
    doc.font('Helvetica-Bold').fontSize(10).fillColor(primaryColor)
      .text('Shed.ng | Inventory, Invoices and more..', 50 + footerLogoWidth, footerTextY);

    // Add clickable link
    const linkText = 'To create professional and winning invoices visit shed.ng/invoices';
    const linkX = 50 + footerLogoWidth;
    const linkY = footerTextY + 15;
    doc.font('Helvetica').fontSize(9).fillColor('#555555')
      .text(linkText, linkX, linkY, { link: 'https://shed.ng/invoices', underline: false });
